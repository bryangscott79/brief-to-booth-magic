...
): Promise<{ data: unknown; knowledge: KnowledgeUsed | null }> {
    ...
    try {
      const invokePromise = supabase.functions
        ...
        .then(({ data, error }) => {
          ...
          // The `knowledge` sibling says which of the agency's documents
          // informed this element. Dropping it here is what made the step's
          // own "weighted context" claim unverifiable.
          return { data: data.data, knowledge: parseKnowledgeUsed(data) };
        });
...
async function runGenerationJob(
  ...
  onKnowledge?: (knowledge: KnowledgeUsed | null) => void,
  ...
  job.promise = (async () => {
    ...
    for (const elementType of elementsToGenerate) {
      ...
      try {
        ...
        const { data: elementData, knowledge } = await invokeGenerateElementWithRetry(
          {
            elementType,
            briefData,
            existingData,
        ...
        // Report the last element's retrieval upward. The batch shares one
        // knowledge scope, so the last answer describes the run.
        if (knowledge) onKnowledge?.(knowledge);
...
export function ElementDashboard({ projectId }: { projectId: string | null }) {
  ...
  /** What the knowledge base contributed to the most recent generation.
   *  Transient on purpose — it describes a run, not the elements. */
  const [lastKnowledge, setLastKnowledge] = useState<KnowledgeUsed | null>(null);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <KnowledgeUsedBadge knowledge={lastKnowledge} className="mt-0" />
