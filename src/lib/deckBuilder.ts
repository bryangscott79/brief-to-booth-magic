...
export interface DeckBuildOptions {
  ...
  /** Per-slide overrides (deckOps.SlideOverrides) keyed by slide index —
   *  ground / hideLogo / accent / notes. Omit → the style's own answers. Pass
   *  the same map to renderDeckHtml so preview === download. */
  slideOverrides?: Record<string, SlideOverrides> | null;
}

/** Everything one slide is drawn with. Built per slide from its resolution
 *  (deckGround) — a slide with no override gets the base palette + tokens,
 *  so the legacy output is untouched. */
interface SlideCtx {
  res: SlideResolution;
  /** Palette this slide draws with — ground-relative roles for a
   *  paper-native layout on a dark override ground, else the kit's. */
  pal: Palette;
  /** Effective ground, bare hex. Dark-native layouts mix soft text toward it. */
  G: string;
  /** Effective ground, '#hex' — logo treatment lookup. */
  groundCss: string;
  /** Style tokens with this slide's accent override applied. */
  tok: DeckStyleTokens;
  hairline: boolean;
  field: boolean;
  /** Text goes light on this ground (drives the body-master bar colour). */
  darkGround: boolean;
}

const bareHex = (css: string): string => css.replace(/^#/, "").toUpperCase();
...
export async function buildDeckPptx(
  ...
  // The kit palette — masters, logo plates and every non-overridden slide
  // draw with it. Overridden slides get their own SlideCtx palette.
  const basePal = paletteFrom(kit);
  ...
  const grounds = kitGrounds(kit);
  const overrides = opts.slideOverrides ?? {};
  ...
  /** The palette a slide draws with (deckGround.groundPalette, in bare hex). */
  const paletteFor = (res: SlideResolution): Palette => {
    if (!usesGroundPalette(res)) return basePal;
    const gp = groundPalette(
      { primary: "#" + basePal.primary, secondary: "#" + basePal.secondary, ink: "#" + basePal.ink, paper: "#" + basePal.paper },
      res.groundHex,
    );
    return {
      primary: bareHex(gp.primary),
      secondary: bareHex(gp.secondary),
      ink: bareHex(gp.ink),
      paper: bareHex(gp.paper),
      muted: bareHex(gp.muted),
      line: bareHex(gp.line),
    };
  };
  const ctxFor = (res: SlideResolution): SlideCtx => {
    const pal = paletteFor(res);
    return {
      res,
      pal,
      G: bareHex(res.groundHex),
      groundCss: res.groundHex,
      tok: res.tok,
      hairline: res.tok.accent.intensity === "hairline",
      field: res.tok.accent.intensity === "field",
      darkGround: usesGroundPalette(res) && pal.paper !== basePal.paper && pal.ink === basePal.paper,
    };
  };
  ...
  pres.defineSlideMaster({
    title: MASTER.cover,
    background: { color: coverOnPaper(tok) ? basePal.paper : basePal.primary },
    objects:
      tok.cover === "grid"
        ? [{ rect: { x: 0, y: 0, w: DECK_PAGE.w, h: 0.09, fill: { color: basePal.primary } } }]
        : [],
  });
  ...
  pres.defineSlideMaster({
    ...
    background: { color: sectionOnPaper(tok) ? basePal.paper : basePal.ink },
    objects:
      tok.section === "number" || tok.section === "index"
        ? [{ rect: { x: 0, y: 0, w: DECK_PAGE.w, h: 0.09, fill: { color: basePal.secondary } } }]
        : [],
  });
  ...
  /** Body-master furniture for a ground + palette. The legacy body master
   *  and every override body master are built from this so they agree. */
  const bodyMasterObjects = (
    pal: Palette,
    topBar: string | null,
    withBareCoMark: boolean,
  ): PptxGenJS.SlideMasterProps["objects"] => {
    const objects: PptxGenJS.SlideMasterProps["objects"] = [
      ...(topBar ? [{ rect: { x: 0, y: 0, w: DECK_PAGE.w, h: 0.09, fill: { color: topBar } } }] : []),
  ...
  const coFooterPlated = !!coLogo && logoTreatmentAt(treat, "co", "footer", grounds.paper) !== "none";
  pres.defineSlideMaster({
    title: MASTER.body,
    background: { color: basePal.paper },
    objects: bodyMasterObjects(basePal, tok.accent.topBar ? basePal.primary : null, !coFooterPlated),
    slideNumber: {
      x: 12.55, y: 7.08, w: 0.4, h: 0.3,
      fontFace: BODY, fontSize: C(8), color: basePal.muted, align: "right",
    },
  });
  ...
  pres.defineSlideMaster({
    title: MASTER.image,
    background: { color: basePal.ink },
    objects: [],
  });

  // ── Override masters ───────────────────────────────────────────────────
  // A slide whose override changes its ground, logo or accent can't share
  // the style's masters (they carry the ground colour and, for body slides,
  // the bare co-mark + bar). One master per distinct combination is
  // defined on first use; decks with no overrides never define any.
  type MasterFamily = "cover" | "section" | "body" | "image";
  const definedMasters = new Set<string>();
  const ensureOverrideMaster = (family: MasterFamily, c: SlideCtx): string => {
    const name = [
      "CANOPY_OV",
      family,
      c.G,
      c.res.hideLogo ? "NOLOGO" : "LOGO",
      c.tok.accent.topBar ? "BAR" : "NOBAR",
      family === "section" ? c.res.section : "",
    ].join("_");
    if (definedMasters.has(name)) return name;
    definedMasters.add(name);
    switch (family) {
      case "cover":
      case "image":
        // The grid cover's brand bar is drawn per slide (drawCover).
        pres.defineSlideMaster({ title: name, background: { color: c.G }, objects: [] });
        break;
      case "section":
        pres.defineSlideMaster({
          title: name,
          ...
          objects:
            c.res.section === "number" || c.res.section === "index"
              ? [{ rect: { x: 0, y: 0, w: DECK_PAGE.w, h: 0.09, fill: { color: basePal.secondary } } }]
              : [],
        });
        break;
      case "body": {
        const bareCo =
          !c.res.hideLogo && logoTreatmentAt(treat, "co", "footer", c.groundCss) === "none";
        // Mirrors deckGround.topBarHex: primary on paper, secondary on a dark ground.
        const topBar = c.tok.accent.topBar ? (c.darkGround ? basePal.secondary : c.pal.primary) : null;
        pres.defineSlideMaster({
          title: name,
          background: { color: c.G },
          objects: bodyMasterObjects(c.pal, topBar, bareCo),
          slideNumber: {
            x: 12.55, y: 7.08, w: 0.4, h: 0.3,
            fontFace: BODY, fontSize: C(8), color: c.pal.muted, align: "right",
          },
        });
        break;
      }
   ...
   *  as before. The treatment is looked up for the slide's EFFECTIVE
   *  ground; plates are always the kit's absolute paper / ink; hideLogo
   *  suppresses the mark. Mirrors deckSlideHtml.logo. */
  const addLogo = (
    ...
    which: "lead" | "co",
    ground: LogoGround,
    c: Pick<SlideCtx, "res" | "groundCss">,
    lockup = false,
  ) => {
    if (!data || c.res.hideLogo) return;
    const aspect = which === "lead" ? treat.leadAspect : treat.coAspect;
    const treatment = logoTreatmentAt(treat, which, ground, c.groundCss);
    ...
    if (treatment !== "none") {
      ...
      slide.addShape("roundRect", {
        ...
        fill: { color: treatment === "plate-ink" ? basePal.ink : basePal.paper },
      });
    }
  ...
  // ── Layout renderers ───────────────────────────────────────────────────
  // Built per slide from its SlideCtx: `pal` is the palette the slide draws
  // with, `tok` its accent-adjusted tokens, `G` its ground (dark-native
  // layouts mix their soft text toward it). Geometry never changes.
  const drawersFor = (c: SlideCtx) => {
    const { pal, tok, hairline, field, G } = c;
    ...
    const drawCover = (s: PptxGenJS.Slide, d: CoverSlide) => {
      ...
      switch (c.res.cover) {
        case "quiet": {
          addLogo(s, leadLogo, COVER_LEAD_BOX, "left", "lead", "cover", c);
          ...
          addLogo(s, coLogo, COVER_CO_BOX, "right", "co", "cover", c);
          return;
        }
        case "editorial": {
          s.addText(d.eyebrow.toUpperCase(), {
          ...
          addLogo(s, leadLogo, COVER_LEAD_BOX_RIGHT, "right", "lead", "cover", c);
        ...
        case "grid": {
          // The style's cover master carries the brand bar; an override
          // master is bare, so draw it here (deckSlideHtml always inlines it).
          if (c.res.overridden) {
            s.addShape("rect", { x: 0, y: 0, w: DECK_PAGE.w, h: 0.09, fill: { color: pal.primary } });
          }
          addLogo(s, leadLogo, COVER_LEAD_BOX, "left", "lead", "cover", c);
          s.addText(d.eyebrow.toUpperCase(), {
        ...
        default: {
          ...
          addLogo(s, leadLogo, COVER_LEAD_BOX, "left", "lead", "cover", c, true);
          s.addShape("rect", { x: M + 0.02, y: 2.62, w: 0.9, h: 0.05, fill: { color: pal.secondary } });
          s.addText(d.eyebrow.toUpperCase(), {
            x: M, y: 2.78, w: 10.5, h: 0.4, fontFace: BODY, fontSize: C(11),
            color: mixHex(pal.paper, G, 0.78), charSpacing: 3, bold: true,
          });
          s.addText(d.title, {
          ...
          s.addText(d.subtitle, {
            ...
            color: mixHex(pal.paper, G, 0.9),
          });
          s.addText(footer, {
            x: M, y: 6.78, w: 8, h: 0.35, fontFace: BODY, fontSize: C(10),
            color: mixHex(pal.paper, G, 0.65), charSpacing: 1,
          });
          addLogo(s, coLogo, COVER_CO_BOX, "right", "co", "cover", c);
          return;
        }
      ...
      switch (c.res.section) {
        ...
        case "index": {
          ...
          if (d.subtitle) {
            s.addText(d.subtitle, {
              ...
              color: mixHex(pal.paper, G, 0.62),
            });
          }
          s.addShape("rect", { x: M, y: 6.3, w: CW, h: 0.008, fill: { color: mixHex(pal.paper, G, 0.25) } });
          s.addText(sectionMetaLine(spec.meta), {
            x: M, y: 6.42, w: CW, h: 0.35, fontFace: BODY, fontSize: C(9),
            color: mixHex(pal.paper, G, 0.6), charSpacing: 2, bold: true,
          });
          return;
        }
        case "number":
        default: {
          s.addText(num, {
            ...
            color: mixHex(pal.secondary, G, 0.42), align: "right", valign: "middle",
          });
    ...
    const drawClosing = (s: PptxGenJS.Slide, d: ClosingSlide) => {
      ...
      if (d.subline) {
        ...
        s.addText(d.subline, {
          ...
          color: onPaper ? pal.ink : mixHex(pal.paper, G, 0.88),
        });
      }
      let y = 4.85;
      for (const contact of d.contacts.slice(0, 3)) {
        const rest = [contact.email, contact.phone].filter(Boolean);
        s.addText(
          [
            { text: contact.name, options: { bold: true, color: onPaper ? pal.primary : pal.paper } },
            {
              text: rest.length ? "   ·   " + rest.join("   ·   ") : "",
              options: { color: onPaper ? pal.muted : mixHex(pal.paper, G, 0.72) },
            },
          ],
      ...
      addLogo(s, leadLogo, CLOSING_LEAD_BOX, "left", "lead", "closing", c);
      addLogo(s, coLogo, CLOSING_CO_BOX, "right", "co", "closing", c);
    };
    ...
    const drawVideo = (s: PptxGenJS.Slide, d: VideoSlide) => {
      ...
      // Letterbox frame: the kit's absolute ink, even on a swapped palette.
      s.addShape("rect", { ...frame, fill: { color: basePal.ink } });
  ...
  // ── Assemble ───────────────────────────────────────────────────────────
  /** Which master family a slide belongs to (mirrors deckGround.onBodyMaster
   *  for the body / image split). */
  const familyFor = (slide: SlideSpec, res: SlideResolution): MasterFamily => {
    switch (slide.layout) {
      case "cover":
      case "closing":
        return "cover";
      case "section":
        return "section";
      case "renderFull":
        return res.framing === "inset" ? "body" : "image";
      default:
        return "body";
    }
  };
  ...
  spec.slides.forEach((slideSpec, i) => {
    const res = resolveSlide(slideSpec, tok, grounds, overrides[String(i)]);
    const c = ctxFor(res);
    const family = familyFor(slideSpec, res);
    const masterName = res.overridden ? ensureOverrideMaster(family, c) : MASTER[family];
    const slide = pres.addSlide({ masterName });
    const draw = drawersFor(c);
    switch (slideSpec.layout) {
      case "cover": draw.drawCover(slide, slideSpec); break;
      case "section": draw.drawSection(slide, slideSpec); break;
      case "briefSummary": draw.drawBriefSummary(slide, slideSpec); break;
      case "concept": draw.drawConcept(slide, slideSpec); break;
      case "elementGrid": draw.drawElementGrid(slide, slideSpec); break;
      case "spatial": draw.drawSpatial(slide, slideSpec); break;
      case "renderFull": draw.drawRenderFull(slide, slideSpec); break;
      case "renderGrid": draw.drawRenderGrid(slide, slideSpec); break;
      case "video": draw.drawVideo(slide, slideSpec); break;
      case "budget": draw.drawBudget(slide, slideSpec); break;
      case "materials": draw.drawMaterials(slide, slideSpec); break;
      case "nextSteps": draw.drawNextSteps(slide, slideSpec); break;
      case "closing": draw.drawClosing(slide, slideSpec); break;
    }
    // Plated footer co-brand mark (bare marks sit on the master instead),
    // decided for THIS slide's ground.
    if (coLogo && family === "body" && !res.hideLogo && logoTreatmentAt(treat, "co", "footer", c.groundCss) !== "none") {
      addLogo(slide, coLogo, FOOTER_CO_BOX, "right", "co", "footer", c);
    }
    if (res.notes) slide.addNotes(res.notes);
